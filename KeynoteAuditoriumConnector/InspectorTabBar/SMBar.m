...
- (id)initWithFrame:(NSRect)frame {
    ...
    if (self) {
		...
		keyGradient = [[NSGradient alloc] initWithStartingColor:color1 endingColor:color2];
		keyBorderColor = [[NSColor colorWithCalibratedWhite:0.416 alpha:1] retain];
		keyLighterBorderColor = [[NSColor colorWithCalibratedWhite:0.480 alpha:1] retain];
		color1 = [NSColor colorWithCalibratedWhite:0.965 alpha:1];
		color2 = [NSColor colorWithCalibratedWhite:0.851 alpha:1];
		nonKeyGradient = [[NSGradient alloc] initWithStartingColor:color1 endingColor:color2];
		nonKeyBorderColor = [[NSColor colorWithCalibratedWhite:0.651 alpha:1] retain];
		nonKeyLighterBorderColor = [[NSColor colorWithCalibratedWhite:0.751 alpha:1] retain];
    }
    return self;
}
...
- (void)dealloc
{
	[keyBorderColor release];
	[nonKeyBorderColor release];
	[super dealloc];
}
...
- (void)drawRect:(CGRect)rect {
    ...
    if ([[self window] isKeyWindow]) {
        // Draw bar gradient
        [keyGradient drawInRect:self.bounds angle:90.0];
        
        // add noise
        [self drawNoisePattern];
        
        // Draw drak gray bottom border
        [keyLighterBorderColor setStroke];
        ...
        // Draw bar gradient
        [nonKeyGradient drawInRect:self.bounds angle:90.0];
        
        // add noise
        [self drawNoisePattern];
        
        // Draw drak gray bottom border
        [nonKeyLighterBorderColor setStroke];
        [NSBezierPath setDefaultLineWidth:0.0f];
        [NSBezierPath strokeLineFromPoint:NSMakePoint(NSMinX(self.bounds), NSMinY(self.bounds) + 0.5f)
                                  toPoint:NSMakePoint(NSMaxX(self.bounds), NSMinY(self.bounds) + 0.5f)];
        [nonKeyBorderColor setStroke];
        [NSBezierPath strokeLineFromPoint:NSMakePoint(NSMinX(self.bounds), NSMaxY(self.bounds) - 0.5f)
                                  toPoint:NSMakePoint(NSMaxX(self.bounds), NSMaxY(self.bounds) - 0.5f)];
    }
}
